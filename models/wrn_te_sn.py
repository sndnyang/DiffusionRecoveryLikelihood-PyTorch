...
class wide_basic(nn.Module):
    def __init__(self, in_planes, planes, dropout_rate, stride=1, norm=None, leak=.2):
        ...
        self.conv1 = sn(nn.Conv2d(in_planes, planes, kernel_size=3, padding=1, bias=True))
        self.dropout = Identity() if dropout_rate == 0.0 else nn.Dropout(p=dropout_rate)
        self.bn2 = get_norm(planes, norm)
        self.conv2 = sn(nn.Conv2d(planes, planes, kernel_size=3, stride=stride, padding=1, bias=True))

        self.temb_dense = sn(nn.Linear(512, planes))

        self.shortcut = nn.Sequential()
        if stride != 1 or in_planes != planes:
            self.shortcut = nn.Sequential(
                sn(nn.Conv2d(in_planes, planes, kernel_size=1, stride=stride, bias=True)),
            )
...
class Wide_ResNet(nn.Module):
    def __init__(self, depth, widen_factor, num_classes=10, input_channels=3,
        ...
        print('| Wide-Resnet %dx%d, sn time embedding' % (depth, k))
        nStages = [16, 16 * k, 32 * k, 64 * k]

        self.layer_one_out = None
        self.conv1 = sn(conv3x3(input_channels, nStages[0]))
        ...
        self.linear = sn(nn.Linear(nStages[3], num_classes))
        self.temb_dense_0 = sn(nn.Linear(128, 512))
        self.temb_dense_1 = sn(nn.Linear(512, 512))
        self.temb_dense_2 = sn(nn.Linear(512, nStages[3]))
